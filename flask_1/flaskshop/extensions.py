...
#  Error     _QueryProperty
# from flask_sqlalchemy import DefaultMeta, Model, SQLAlchemy, _QueryProperty

# Use flask_sqlalchemy.model._QueryProperty.__get__ instead of
# flask_sqlalchemy._QueryProperty.__get__. 
# This will resolve your issue as _QueryProperty class has been moved into model.

from flask_sqlalchemy import DefaultMeta, Model, SQLAlchemy
...
# Проблема не решена 
#! @babel.localeselector  не работает 
def get_locale():
    if request.args.get("lang"):
        session["lang"] = request.args.get("lang")
    return session.get("lang", "en")

# добавил  https://stackoverflow.com/questions/75270265/attributeerror-babel-object-has-no-attribute-localeselector
# babel.init_app(current_app, locale_selector=get_locale)
...
class UnLockedAlchemy(SQLAlchemy):
    def make_declarative_base(self, model, metadata=None):
        ...
        # возможно тут проблема со свойствами которые теперь не извлекаются
        # c большой буквы  Model !
        model.query = Model._QueryProperty(self)
        return model
